    ...
    private func setupLevelUpNotification() {
        NotificationCenter.default.addObserver(
            ...
            queue: .main) { notification in
                if let level = notification.userInfo?["level"] as? Level {
                    newLevelName = level.name
                    showingLevelUpAlert = true
                }
            }
    }
    
    private func shareScore() {
        let scoreText = "I scored \(game.score) out of \(game.getTotalQuestions()) in \(game.currentGameMode.rawValue) on Triviaholic! Current level: \(levelManager.getCurrentLevel().name) 🎮🧠"
